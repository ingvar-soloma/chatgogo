...
func (m *ManagerService) handleStopCommand(message models.ChatMessage) {
	...
	if len(complaints) == 0 && len(complaints2) == 0 {
		...
		if time.Since(room.StartedAt) > 10*time.Minute {
			user1Messages := 0
			user2Messages := 0
			...
			if user1Messages >= 10 && user2Messages >= 10 {
				// Successful dialog
				m.Storage.UpdateUserReputation(room.User1ID, 1)
				m.Storage.UpdateUserReputation(room.User2ID, 1)
			}
		} else if time.Since(room.StartedAt) < 2*time.Minute {
			// Early disconnect
			partnerMessages := 0
			var partnerID string
			if message.SenderID == room.User1ID {
			...
			if partnerMessages < 2 {
				m.Storage.UpdateUserReputation(message.SenderID, -1)
			}
		}
	}
}
